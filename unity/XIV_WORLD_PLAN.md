...
The current builder already creates the first Green Gate and Archive Garden landmarks, authored first-pass Systems and Review spaces for Semiconductor Speedway and Earnings Arcade, an authored procedural sky with depth fog, a dressed route blockout, Rosco interest points, subtle wind motion for route props, and a local `XIVAudioAtmosphere` component. If the reviewed `Assets/Art/Exports/GreenGate.fbx` exists, the builder uses it in place of the Green Gate blockout without changing the sign, lighting, or validation contract. Interest points pulse as small route landmarks and change state when Rosco discovers them. The in-world guide returns to an Archive Garden distance cue after each discovery, reflects Rosco waiting or investigating, and resolves to the saved-walk state at the destination. `XIVWalkSession` now keeps a local history of completed walks so Archive Garden can remember more than the current launch; a failed destination save leaves the trigger retryable instead of presenting an unsaved walk as complete. The private build also has a native pause state that freezes time and audio and restores both on resume, plus a reproducible XIV product identity for the macOS build. Music energy now reaches both the light response and the route motion props, with beat accents kept deliberately restrained; an explicit audio path remains preferred, with a local `XIV/Audio` folder as the first-run fallback. `RoscoCompanion` uses a distance band for natural catch-up, can rejoin Marcelo after a large transfer, and now drives an expressive procedural bridge for idle breathing, walking, curiosity, and celebration while the skinned asset is still in production. The third-person camera frames Marcelo and Rosco as a pair and expands its distance slightly when they separate. A final skinned Rosco can replace the placeholder without rewriting the route logic. These are foundations for the gates below, not evidence that the final art or audio experience is complete.
