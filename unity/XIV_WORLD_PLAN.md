...
The current builder already creates the first Green Gate and Archive Garden landmarks, a dressed route blockout, Rosco interest points, and a local `XIVAudioAtmosphere` component. Interest points pulse as small route landmarks and change state when Rosco discovers them. `RoscoCompanion` also exposes an optional animator handoff for speed, state, investigation, and celebration, so a final skinned Rosco can replace the placeholder without rewriting the route logic. These are foundations for the gates below, not evidence that the final art or audio experience is complete.
