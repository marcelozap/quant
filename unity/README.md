...
Use `WASD` or the arrow keys to walk, and hold `Shift` to run. Hold the right mouse button and drag to orbit the camera, or use `Q` and `E`. The camera gently frames Marcelo and Rosco together, with collision protection around the pair. A left click on the ground sets a temporary walk destination. Press `F` to ask Rosco to wait or resume, and `R` to call him back. The in-world guide keeps the Archive Garden route visible, reflects Rosco's current moment, and settles on the saved-walk state at the destination. The first generated scene is a systems blockout, so the player and Rosco are intentionally placeholders while the route and behavior are being tested.
