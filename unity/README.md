...
Archive Garden also reads an optional local XIV/archive.json file for songs, projects, and memories. Each entry uses category, title, note, and date; a missing or unavailable file produces an explicit empty state and never prevents the walk from running.
