...
1. Install Unity Hub and Unity `6000.0.82f1` with the build support for the machine you are testing on.
...
The editor action creates the first exterior world shell, bakes a NavMesh for the park grounds, and adds a third-person player, an expressive Rosco blockout, an authored procedural sky with depth fog, lighting, and eight named landmarks. Rosco's procedural bridge gives the placeholder a breathing idle, alternating walk cycle, curious investigation pose, tail wag, and small celebration response; the final skinned asset can keep the same companion contract. The authored time of day is saved locally in the application's `XIV` folder and restored on the next launch; if that state is unavailable, XIV uses its default morning hour. It does not store data or secrets in Unity.

Use `WASD` or the arrow keys to walk, and hold `Shift` to run. Hold the right mouse button and drag to orbit the camera, or use `J` and `L`. The camera gently frames Marcelo and Rosco together, with collision protection around the pair. A left click on the ground sets a temporary walk destination. Near a route discovery, press `E` to inspect it and receive a short authored observation; each interaction is counted in the local walk session. Press `F` to ask Rosco to wait or resume, `R` to call him back, and `Esc` to pause the private build. If Marcelo falls below the test ground, the controller returns him to Green Gate and clears movement so the walk can continue. The in-world guide keeps the Archive Garden route visible, while the departure arch and audio-reactive waystones carry the route through the environment. The guide reflects Rosco's current moment and settles on the saved-walk state at the destination. The first generated scene is a systems blockout, so the player and Rosco are intentionally placeholders while the route and behavior are being tested.
...
The Green Gate source lives at `Assets/Art/Source/GreenGate.blend`, with the repeatable generator at `Assets/Art/Source/generate_green_gate.py`. From the repository root, run `blender --background --python unity/Assets/Art/Source/generate_green_gate.py` on a machine with Blender installed. It writes the Blender source, a preview, and the Unity-ready FBX at `unity/Assets/Art/Exports/GreenGate.fbx`. Once that FBX is reviewed, the world builder automatically instantiates it in place of the procedural Green Gate blockout while preserving the XIV sign, lighting, and validator contract.

Marcelo has the same reversible art handoff: run `blender --background --python unity/Assets/Art/Source/generate_marcelo.py` to create `Assets/Art/Source/MarceloHero.blend` and `Assets/Art/Exports/MarceloHero.fbx`. The builder uses that FBX automatically when it exists and falls back to the procedural silhouette when it does not. The player root, movement, camera, and `MarceloProceduralAnimator` contract remain unchanged.

The generated world also includes `XIVAudioAtmosphere`, `XIVWalkSession`, `XIVWalkDestination`, a Green Machine read-only board, and an XIV Systems board. Reaching Archive Garden completes the first walk, records the destination, celebrates with Rosco, and saves both the current session and a local walk history; the Archive Garden summary keeps the total walk count. Set `XIV_AUDIO_PATH` to a local MP3, WAV, or OGG file, or place a track in the app's local `XIV/Audio` folder; the explicit path wins, and the first alphabetically sorted local track is the fallback. The clip is analyzed locally and its energy is sent to world lights, emissive beacons, and restrained route motion. MaloSound can provide an explicit BPM and offset through `SetBeatGrid`, or XIV can load the exact `AudioAnalysisV1` JSON shape from `XIV_AUDIO_ANALYSIS_PATH`; when `beat_times` is present, XIV preserves that timestamped beat timeline instead of collapsing it to a constant grid. Invalid or missing timing data disables beat pulses rather than guessing. The Green Machine board reads `/world/today` from the loopback API when available and displays an offline state otherwise. The XIV Systems board reads an optional local `XIV/systems.json` file and falls back to the three-lane identity. Walk summaries and history are autosaved to the local application's data directory on whichever machine runs the build. Audio files, session saves, tokens, and private data should stay outside Git.

Archive Garden also reads an optional local XIV/archive.json file for songs, projects, and memories. Each entry uses category, title, note, and date; a missing or unavailable file produces an explicit empty state and never prevents the walk from running. The safe shape is shown in Examples/archive.example.json; copy it into the application's local XIV folder and replace the sample text there, without committing the personal copy.
